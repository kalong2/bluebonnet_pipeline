...
sub detect_indel {
	...
	my $original_seq;
	my $modified_seq;
	if ($length2 < $length1) { #if ( length $seq2 < length $seq1 ) {
		$original_seq  = $seq1;
		$modified_seq  = $seq2;
		$order_reverse = 1;
	}
	...
	if ( $order_reverse == 0 ) {
		...
		if (length $right_flank_modified == 0){
		    my $len = length($aa_change_original);
		    my $remaining_length = $len - 20;
		    if ($remaining_length > 0) {
			$aa_change_original = substr($aa_change_original,0,20). "...(+" . $remaining_length . " amino acids)...*";
		    } else {
		...
		if (length $right_flank_modified == 0){
		    my $len = length($aa_change_modified);
		    my $remaining_length = $len - 20;
		    if ($remaining_length > 0) {
                        $aa_change_modified = substr($aa_change_modified,0,20)."...(+" . $remaining_length . " amino acids)...*";
		    } else {
