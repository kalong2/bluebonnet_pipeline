...
$bin = "/work/NBS/SIFT/SIFTINDEL/bin";
...
#	Date:     $Date: 2011/03/15 08:18:32 $
#	Author:   $Author: simnl $
#
######################################################################

my $SPLASH="Copyright (c) 2005 J. Craig Venter Institute.\nAll rights reserved.";
my $VERS_INFO=qw$Revision: 1.3 $;
...
#my $FLANKING_LENGTH=500; # SIM TEST
...
# print NORM_OUT_FH "Foreach $id allele ref: $ref_allele mutated_allele: $allele end $snp_ori\n";
		...
		$allele =~ s/\s+//g;
	#	print "ref_allel: ".$ref_allele." allele: ".$allele;	
...
#March 21, 2013 only put location restriction for non3n indels Pauline and Jing
			if ($begin <= 50 && $allele_length % 3 !=0) {
...
#			print NORM_OUT_FH "adjacent introns $distance_to_upstream_intron  $adjacent_upstream_intron_size  $distance_to_downstream_intron  $adjacent_downstream_intron_size\n";
			...
			if($frameshift_mut){
				...
				#Added by Jing to decide whether the 3n indel causes early stop, aa insertion/deletion or synonymous
				
				my $expectedAASizeChange = $target_length;
				if ($target_length==0){
					$expectedAASizeChange = $allele_length;  #if it is an insertion
					}
				$expectedAASizeChange = $expectedAASizeChange / 3;
				
				# 2) actual AA sequence length change = orignal length - new protein length
				my $actualAAsizeChange =  abs($refer_protein_length - $donor_prot_len);  #this is the actual AA size change
				
				if ($refer_protein eq $donor_protein){
         				$mutation_type="SYNONYMOUS[$refer_prot_len $donor_prot_len]";
     				} 
				elsif ($expectedAASizeChange == $actualAAsizeChange) {
				...
				else {# expected AA SizeChange != actual AAsizeChange
  					$mutation_type="EARLYSTOP[$refer_prot_len $donor_prot_len]";
				}
