...
use lib '/usr/local/lib/perl5/site_perl/5.12.0';
...
#my $Coding_info_db_dir = $ARGV[3];
my $transcript_database = $ARGV[3];
my $enst_content;
my @result_row;
my @rows;
#my $db = DBI->connect( "dbi:SQLite:dbname=$Coding_info_db_dir/TRANSCRIPT_DB.sqlite","",  "", { RaiseError => 1, AutoCommit => 1} );
my $db = DBI->connect( "dbi:SQLite:dbname=$transcript_database","",  "", { RaiseError => 1, AutoCommit => 1} );
